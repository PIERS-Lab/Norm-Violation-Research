...
class vecDeliv:
    def __init__ (self, serialNum, cube_Num):
        ...
        self._robot = anki_vector.Robot(serialNum)
        self._robot.connect()
    ...
    async def create(serialNum, cube_num):
        ...
        self._goals = [await self._robot.world.define_custom_wall(CustomObjectTypes.CustomType01,
                                              ...
                                              100, 120,
                                              40, 40, True), await self._robot.world.define_custom_wall(CustomObjectTypes.CustomType03,
    ...
    async def failmsg(self, detail = "."):
        await self._robot.behavior.say_text("I can't do that! " + detail).wait_for_completed()
    # It needs to take input as apart of analyzing the task
    # returns the found lightcube object
    async def findCube(self, cbID):
        await self._robot.behavior.set_head_angle(degrees(0)).wait_for_completed()
        ...
        currBehavior = self._robot.behavior.look_around_in_place()
        secs = 0
        sTime = time.clock_gettime(1)
        try: 
            ''' FINISH AND TEST'''
            while(True):
                if (self.latestSeen.obj == anki_vector.world.World.light_cube):
            ...
            await self._robot.behavior.say_text("I couldn't find my cube").wait_for_completed()
            return False
    ...
    async def lift_cube(self, target):
        await self._robot.behavior.dock_with_cube(target, num_retries=3, approach_angle=anki_vector.util.degrees(0)).wait_for_completed()
        await self._robot.behavior.set_lift_height(1.0).wait_for_completed()

    async def drop_cube(self):
        await self._robot.behavior.set_lift_height(0).wait_for_completed()
        # back away from cube to avoid messing with it accidentally
        await self._robot.behavior.drive_straight(distance_mm(-100), anki_vector.util.speed_mmps(100)).wait_for_completed()
